...
int main(int argc, char** argv) {
  ...
  viewer->update_drawable("dense", cloud_buffer, guik::VertexColor().set_point_scale(0.05f));
  ...
  for (rosbag::MessageInstance const m : rosbag::View(bag, rosbag::TopicQuery("/os_cloud_node/points"))) {
    ...
    viewer->update_drawable("sparse", cloud_buffer, guik::VertexColor().set_point_scale(0.05f));
