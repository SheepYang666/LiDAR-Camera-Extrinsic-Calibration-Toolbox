...
public:
  ...
  void ui_callback() {
    ...
    if (ImGui::Button("Calibrate")) {
      ...
      sub->update_drawable("points", color_updater->cloud_buffer, guik::VertexColor().set_point_scale(0.05f));
...
private:
  void detect_sphere(const Eigen::Vector3f& clicked_pt_) {
    ...
    auto viewer = guik::LightViewer::instance();
    viewer->update_drawable("selected", std::make_shared<glk::PointCloudBuffer>(filtered->points, filtered->size()), guik::FlatOrange().set_point_scale(0.05f));
