...
void StaticPointCloudIntegrator::insert_points(const Frame::ConstPtr& raw_points) {
  ...
  if (params.visualize) {
    ...
    viewer->update_drawable(guik::anon(), cloud_buffer, guik::Rainbow().set_point_scale(0.05f));
    viewer->update_drawable("current", cloud_buffer, guik::FlatOrange().set_point_scale(0.05f));
    viewer->spin_once();
  }
}
