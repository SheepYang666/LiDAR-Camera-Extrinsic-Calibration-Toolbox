...
bool Preprocess::run(int argc, char** argv) {
  ...
  description.add_options()
    ...
    ("config", value<std::string>(), "YAML config with paths + preprocess sections")
  ...
  // Peek for --config so YAML can supply defaults; CLI values stored afterwards override YAML.
  variables_map vm_peek;
  store(command_line_parser(argc, argv).options(description).positional(p).allow_unregistered().run(), vm_peek);
  notify(vm_peek);

  if (vm_peek.count("help")) {
  ...
  if (vm_peek.count("config")) {
    const std::string config_path = vm_peek["config"].as<std::string>();
    std::cout << "config: " << config_path << std::endl;
    try {
      const auto yaml_tokens = preprocess_args_from_yaml(config_path);
      std::vector<std::string> yaml_argv_storage;
      yaml_argv_storage.reserve(yaml_tokens.size() + 1);
      yaml_argv_storage.emplace_back(argv[0]);
      yaml_argv_storage.insert(yaml_argv_storage.end(), yaml_tokens.begin(), yaml_tokens.end());

      std::vector<char*> yaml_argv;
      yaml_argv.reserve(yaml_argv_storage.size());
      for (auto& token : yaml_argv_storage) {
        yaml_argv.push_back(token.data());
      }

      store(command_line_parser(static_cast<int>(yaml_argv.size()), yaml_argv.data()).options(description).run(), vm);
    } catch (const std::exception& e) {
      std::cerr << vlcal::console::bold_red << "error: " << e.what() << vlcal::console::reset << std::endl;
      return false;
    }
  }
  ...
  if (!vm.count("data_path") || !vm.count("dst_path")) {
    std::cout << description << std::endl;
    return true;
  }
...
#pragma omp parallel for
  ...
  if (vm.count("visualize")) {
    ...
    for (const auto& bag_filename : bag_filenames) {
      ...
      viewer->update_drawable("points", cloud_buffer, guik::VertexColor().set_point_scale(0.05f));
