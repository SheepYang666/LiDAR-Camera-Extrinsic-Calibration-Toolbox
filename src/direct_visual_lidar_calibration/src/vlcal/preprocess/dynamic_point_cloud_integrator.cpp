...
#pragma omp parallel for num_threads(params.num_threads)
  ...
  if (params.visualize) {
    ...
    viewer->update_drawable("points", std::make_shared<glk::PointCloudBuffer>(deskewed->points, deskewed->size()), guik::FlatOrange().set_point_scale(0.05f));
    viewer->update_drawable("target", std::make_shared<glk::PointCloudBuffer>(target_ivox->voxel_points()), guik::Rainbow().set_point_scale(0.05f));
    viewer->spin_once();
  }
}
