...
Eigen::Isometry3d VisualCameraCalibration::estimate_pose_nelder_mead(const Eigen::Isometry3d& init_T_camera_lidar) {
  ...
  for (const auto& data : dataset) {
    ...
    viewer->invoke([=] {
      ...
      sub->update_drawable("raw", raw_cloud_buffer, guik::FlatColor(0.2f, 0.2f, 0.2f, 1.0f).set_point_scale(0.05f));
      sub->update_drawable("culled", culled_cloud_buffer, guik::VertexColor().set_point_scale(0.05f));
    });
    */
  }
