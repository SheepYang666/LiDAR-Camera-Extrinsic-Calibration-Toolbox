...
void VisualLiDARVisualizer::ui_callback() {
  ...
  if (prev_selected_bag_id != selected_bag_id) {
    ...
    viewer->update_drawable("points", color_updater->cloud_buffer, guik::VertexColor().set_point_scale(0.05f));

    if (draw_sphere) {
      sphere_updater.reset(new PointsColorUpdater(proj, dataset[selected_bag_id]->image));
      viewer->update_drawable("sphere", sphere_updater->cloud_buffer, guik::VertexColor().set_point_scale(0.05f));
    }
